...
const allItemsInCatagory = (req,res) => {
    const {id} = req.params
    pool.query('SELECT P.ID AS "Product ID", P.name AS "Product Name", P.price AS "Price", P.Image AS "Image", P.Weight AS "Weight", P.description AS "description", C.name AS "Catagory Name" FROM Product AS P, Catagory AS C, Product_Catagory AS PC WHERE PC.product_id = P.id AND PC.catagory_id = C.id AND C.id = $1', [id], (error, results) => {
        if (error){
            throw error
        }
        res.status(200).json(results.rows)
